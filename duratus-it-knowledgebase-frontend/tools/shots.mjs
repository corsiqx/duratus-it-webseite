...
// The dev server must run on :4202.
//
// Usage: node tools/shots.mjs <outDir> <width> <height> <step> [<step> ...]
// Steps:
//   login                 sign in with the demo session (sessionStorage) and reset demo data
...
const profile = join(tmpdir(), `kb-shots-${port}`);
...
try {
  for (const step of steps) {
    ...
    const arg = rest.join(':');
    if (command === 'login') {
      await send('Page.navigate', { url: `${BASE}/anmelden` });
      await sleep(1500);
      await evaluate(`localStorage.clear(); sessionStorage.setItem('duratus-wissensdatenbank-session', JSON.stringify({ email: 'j.hoffmann@duratus-it.de', since: new Date().toISOString() }));`);
      ...
      const issues = await evaluate(`
        ...
        for (const el of document.querySelectorAll('body *')) {
          if (el.closest('#kb-sidebar[inert], .sr-only, [popover]:not(:popover-open), dialog:not([open]), fieldset.overflow-x-auto')) continue;
