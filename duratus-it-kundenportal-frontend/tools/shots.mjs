...
// The dev server must run on :4201.
//
...
const profile = join(tmpdir(), `portal-shots-${port}`);
...
try {
  for (const step of steps) {
    ...
    if (command === 'login') {
      ...
      await sleep(1500);
      await evaluate(`localStorage.clear(); sessionStorage.setItem('duratus-kundenportal-session', JSON.stringify({ email: 'max.mustermann@muster-gmbh.de', since: new Date().toISOString() }));`);
